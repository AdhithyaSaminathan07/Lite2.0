// // In: src/components/forms/loginform.tsx
// 'use client';

// import React, { useState } from 'react';
// import Link from 'next/link';
// import { useRouter } from 'next/navigation';
// import { signIn } from 'next-auth/react'; // <<< 1. IMPORT THIS

// export function LoginForm() {
//   // ... all your existing state and functions (useState, handleChange, etc.) are here and remain unchanged ...
...
//   const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (!validateForm()) return; setIsLoading(true); setErrors({}); try { await new Promise(resolve => setTimeout(resolve, 1500)); if (formData.email === DEMO_CREDENTIALS.email && formData.password === DEMO_CREDENTIALS.password) { if (typeof window !== 'undefined') { localStorage.setItem('isAuthenticated', 'true'); localStorage.setItem('userEmail', formData.email); } router.push('/dashboard'); } else { setErrors({ general: 'Invalid email or password. Use demo credentials.' }); } } catch (error) { setErrors({ general: 'An error occurred. Please try again.' }); } finally { setIsLoading(false); } };
//   const fillDemoCredentials = () => { setFormData({ email: DEMO_CREDENTIALS.email, password: DEMO_CREDENTIALS.password }); setErrors({}); };

//   // <<< 2. ADD THIS NEW FUNCTION FOR THE GOOGLE BUTTON
//   const handleGoogleSignIn = async () => {
//     setIsLoading(true);
//     // This triggers the Google Sign-In flow managed by NextAuth
//     await signIn('google', { callbackUrl: '/dashboard' });
//   };

//   return (
//     <form className="space-y-6" onSubmit={handleSubmit} noValidate>
//       {/* ... Your existing Email, Password, and "Sign In" button are here ... */}
...
//       {/* <<< 3. ADD THE DIVIDER AND THE NEW GOOGLE BUTTON HERE */}
...
//       {/* ... Your Demo Credentials section is here ... */}
