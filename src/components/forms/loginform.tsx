...
export function LoginForm() {
   ...
   * It sets the loading state to true to disable the button and show a loading indicator,
   * then calls the signIn function from NextAuth. Upon success, NextAuth will handle
   * the redirect to the specified callbackUrl.
   */
  const handleGoogleSignIn = async () => {
    setIsLoading(true);
    // This initiates the Google authentication flow.
    // After the user signs in with Google, they will be redirected to the '/dashboard'.
    await signIn('google', { callbackUrl: '/dashboard' });
    // The page will redirect on success, so we don't typically need to set isLoading back to false.
  };

  return (
    <div className="space-y-6">
      {/* 
        This is the primary button for initiating the Google Sign-In flow.
      ...
      */}
      <button
        ...
        onClick={handleGoogleSignIn}
        disabled={isLoading}
        className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-colors duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {isLoading ? (
          // Loading indicator shown while the sign-in process is active.
          <div className="flex items-center justify-center">
            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-gray-700 mr-3"></div>
            <span>Redirecting to Google...</span>
          </div>
        ) : (
          // Default button content with Google icon and text.
          <>
            <svg className="w-5 h-5 mr-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48">
              ...
              <path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.222,0-9.582-3.443-11.113-8.06l-6.571,4.819C9.656,39.663,16.318,44,24,44z"></path>
              <path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571l6.19,5.238C39.988,35.617,44,30.138,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
            </svg>
            Sign in with Google
          </>
        )}
