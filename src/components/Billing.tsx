...
import QRCode from 'react-qr-code';
import {
  Scan, Trash2, Edit2, Check, X, Sun, AlertTriangle,
  Send, CreditCard, CheckCircle, DollarSign, MessageSquare, RefreshCw
} from 'lucide-react';
...
export default function BillingPage() {
  ...
  const merchantName = session?.user?.name || 'Billzzy Lite';
  const upiQR = merchantUpi
    ? `upi://pay?pa=${merchantUpi}&pn=${encodeURIComponent(
        merchantName
      )}&am=${totalAmount.toFixed(2)}&cu=INR&tn=Bill%20Payment`
    : '';
      ...
      <div className="flex flex-col h-screen bg-[#f9f9fb] font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div
                key={item.id}
                className="flex justify-between items-center bg-white rounded-lg shadow p-3"
              >
                <div>
        ...
        <footer className="bg-white p-4 shadow-inner">
          <div className="flex justify-between items-center">
            ...
            <span className="text-lg font-bold text-[#5a4fcf]">
        ...
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80">
          <div className="w-72 rounded-xl bg-white p-3 relative shadow-lg">
            ...
            <Scanner
              ...
              onScan={handleScan}
              scanDelay={150}
              {...({ torch: flashOn } as any)} // ✅ keeps flash functional & removes TS error
              styles={{
                container: { width: '100%', height: 220, position: 'relative' },
              }}
            />
