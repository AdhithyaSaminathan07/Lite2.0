  ...
  const sendWhatsAppMessage = async (phoneNumber: string, messageType: string) => {
    ...
    try {
      ...
      switch (messageType) {
        ...
        case 'cashPayment': templateName = "payment_receipt_cash"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
        case 'qrPayment': templateName = "payment_receipt_upi"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
