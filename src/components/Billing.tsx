...
export default function BillingPage() {
  ...
  }, [scanning]); // This effect only runs when the `scanning` state changes
  ...
  const handleManualAdd = useCallback(() => {
    if (!productName.trim() || !productPrice || productPrice <= 0) {
      setModal({ isOpen: true, title: 'Invalid Input', message: 'Please enter a valid product name and a price greater than zero.', showCancel: false, confirmText: 'OK', onConfirm: undefined });
      return;
    }
    const matchedItem = inventory.find(p => p.name.toLowerCase() === productName.trim().toLowerCase());
    if (matchedItem) {
  ...
  const handleStartNewBill = useCallback(() => {
    setModal({ isOpen: true, title: 'Confirm Action', message: 'Are you sure you want to clear the current bill and start a new one?', showCancel: true, confirmText: 'Yes, Start New', onConfirm: handleTransactionDone });
      ...
      <div className="flex h-screen w-full bg-gray-100 font-sans">
        <div className="flex h-full w-full flex-col md:flex-row overflow-hidden">
          ...
          <div className="flex flex-col p-4 md:w-2/3 md:p-6 flex-1 overflow-y-auto">
            ...
            <div className="flex-shrink-0 rounded-xl bg-white p-4 mb-4 shadow-sm">
              <div ref={suggestionsRef} className="relative">
                <input type="text" placeholder="Search by Product Name or ID..." className="w-full rounded-lg border-2 border-gray-200 p-3 outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" value={productName} onChange={(e) => setProductName(e.target.value)} />
                {showSuggestions && (
                  <div className="absolute z-10 mt-1 w-full rounded-lg border border-gray-200 bg-white shadow-lg">
                    {suggestions.map((s) => (
                      <div key={s.id} onClick={() => addToCart(s.name, s.sellingPrice, s.id)} className="cursor-pointer border-b p-3 last:border-b-0 hover:bg-indigo-50">
                        <div className="flex justify-between font-semibold">
                          <span>{s.name}</span><span>₹{s.sellingPrice.toFixed(2)}</span>
                        </div>
                        {s.sku && <p className="text-xs text-gray-500">ID: {s.sku}</p>}
                      </div>
                    ))}
            ...
            <div className="space-y-3 pr-2">
              {cart.length === 0 ? (
                ...
                cart.map((item) => (
                  <div key={item.id} className="grid grid-cols-12 items-center gap-2 rounded-lg bg-white p-3 shadow-sm">
                    <div className="col-span-12 md:col-span-5"><p className="font-semibold text-gray-800">{item.name}</p><p className="text-sm text-gray-500 md:hidden">Total: ₹{(item.quantity * item.price).toFixed(2)}</p></div>
                    <div className="col-span-5 md:col-span-2 flex items-center"><label htmlFor={`quantity-${item.id}`} className="text-sm font-medium text-gray-500 mr-2">Qty:</label><input id={`quantity-${item.id}`} type="number" value={item.quantity} onChange={(e) => editCartItem(item.id, 'quantity', e.target.value)} className="w-full rounded-md border-2 p-1.5 text-center font-semibold outline-none focus:ring-1 focus:ring-indigo-500" min="1" /></div>
                    <div className="col-span-5 md:col-span-3 flex items-center"><label htmlFor={`price-${item.id}`} className="text-sm font-medium text-gray-500 mr-2">Price:</label><input id={`price-${item.id}`} type="number" value={item.price} onChange={(e) => editCartItem(item.id, 'price', e.target.value)} className="w-full rounded-md border-2 p-1.5 text-right font-semibold outline-none focus:ring-1 focus:ring-indigo-500" /></div>
                    <div className="col-span-2 md:col-span-1 text-right"><button onClick={() => deleteCartItem(item.id)} className="rounded-full p-2 text-red-500 transition-colors hover:bg-red-100 hover:text-red-700"><Trash2 size={20} /></button></div>
                    <div className="hidden md:block col-span-1 text-right font-semibold text-gray-700">₹{(item.quantity * item.price).toFixed(2)}</div>
                  </div>
                ))
              )}
            </div>
          ...
          <div className="flex flex-shrink-0 flex-col border-t bg-white p-4 md:w-1/3 md:border-l md:border-t-0 md:p-6 md:shadow-lg md:overflow-y-auto">
            <div className="flex-grow space-y-4">
              <div className="flex items-center justify-between border-b pb-2"><h2 className="text-xl font-bold text-gray-800">Order Summary</h2><button onClick={handleStartNewBill} className="p-2 text-gray-500 rounded-full hover:bg-gray-200 hover:text-gray-700 transition-colors disabled:text-gray-300 disabled:cursor-not-allowed" title="Start New Bill" disabled={cart.length === 0}><RefreshCw size={20} /></button></div>
              <div className="flex items-center justify-between"><span className="text-lg font-medium text-gray-600">Grand Total</span><span className="text-3xl font-bold text-indigo-600">₹{totalAmount.toFixed(2)}</span></div>
              <div className="space-y-3 border-t pt-4">
                <div className="flex gap-3">
                  <button onClick={() => { setShowFinalizeOptions(p => !p); setShowPaymentOptions(false); }} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-gray-700 py-2 px-3 font-semibold text-white transition-all hover:bg-gray-800 disabled:bg-gray-400" disabled={cart.length === 0}><CheckCircle size={16} /><span>Finalize Bill</span></button>
                  <button onClick={() => { setShowPaymentOptions(p => !p); setShowFinalizeOptions(false); }} className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-blue-500 py-2 px-3 font-semibold text-white transition-all hover:bg-blue-600 disabled:bg-gray-400" disabled={cart.length === 0}><CreditCard size={16} /><span>Accept Payment</span></button>
                </div>
                {showFinalizeOptions && cart.length > 0 && (
                  <div className="space-y-3 rounded-lg bg-gray-50 p-3 pt-2">
              ...
              {showPaymentOptions && cart.length > 0 && (
                <div className="space-y-3 border-t pt-4">
                  <div className="flex flex-wrap gap-2">{['Cash', 'QR Code', 'Card'].map((method) => (<button key={method} onClick={() => setSelectedPayment(method)} className={`rounded-lg px-4 py-2 text-sm font-semibold transition-all ${selectedPayment === method ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>{method}</button>))}</div>
                  {selectedPayment === 'Cash' && (<div className="space-y-3 rounded-lg bg-gray-50 p-4 text-center"><h3 className="font-bold text-gray-800">Confirm Cash Payment</h3><p className="text-sm text-gray-600">Confirm receipt of ₹{totalAmount.toFixed(2)} cash.</p><button onClick={handlePaymentSuccess} className="flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 p-3 font-bold text-white hover:bg-blue-700"><DollarSign size={20} /><span>Cash Received</span></button></div>)}
                  {selectedPayment === 'QR Code' && (<div className="space-y-3 rounded-lg bg-gray-50 p-4 text-center">{upiQR ? (<><h3 className="font-bold text-gray-800">Scan to Pay</h3><div style={{ height: "auto", margin: "0 auto", maxWidth: 200, width: "100%" }}><QRCode size={256} style={{ height: "auto", maxWidth: "100%", width: "100%" }} value={upiQR} viewBox={`0 0 256 256`} /></div><p className="text-sm text-gray-600">Pay to <b>{merchantUpi}</b></p><button onClick={handlePaymentSuccess} className="flex w-full items-center justify-center gap-2 rounded-lg bg-green-600 p-3 font-bold text-white hover:bg-green-700"><CheckCircle size={20} /><span>Payment Received</span></button></>) : (<p className="p-2 font-semibold text-red-600">UPI ID not configured in Settings.</p>)}</div>)}
                  {selectedPayment === 'Card' && (<div className="space-y-3 rounded-lg bg-gray-50 p-4 text-center"><h3 className="font-bold text-gray-800">Confirm Card Payment</h3><p className="text-sm text-gray-600">Confirm transaction was successful on the card machine.</p><button onClick={handlePaymentSuccess} className="flex w-full items-center justify-center gap-2 rounded-lg bg-purple-600 p-3 font-bold text-white hover:bg-purple-700"><CreditCard size={20} /><span>Payment Successful</span></button></div>)}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75">
            <div className="w-full max-w-sm rounded-xl bg-white p-4">
              <div className="mb-2 flex items-center justify-between">
                <h3 className="font-bold text-indigo-600">Scan Barcode/QR</h3>
                <button onClick={() => setScanning(false)} className="rounded-full p-1 hover:bg-gray-200"><X size={24} /></button>
              </div>
              <div id={scannerContainerId} className="overflow-hidden rounded-lg w-full" />
            </div>
          </div>
        )}
      </div>
