...
//         case 'cashPayment': templateName = "payment_receipt_cashhh"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
//         case 'qrPayment': templateName = "payment_receipt_upiii"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
...
//     if (success) { setModal({ isOpen: true, title: 'Bill Shared', message: 'The invoice has been sent to the customer via WhatsApp.', showCancel: false, confirmText: 'OK', onConfirm: undefined }); }
//   };
...
//                   <button onClick={handleWhatsAppShare} disabled={isMessaging || !whatsAppNumber.trim()} className="rounded-lg bg-green-500 p-2 text-white hover:bg-green-600 disabled:bg-green-300 transition-colors" title="Send Bill via WhatsApp">
//                     {isMessaging ? (<div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent" />) : (<Send size={20} />)}
//                   </button>
//                 </div>
//                 {whatsAppNumber.trim() && (
...
//                 )}
//                 <button onClick={handleProceedToPaymentWithWhatsApp} disabled={isMessaging} className="w-full flex items-center justify-center gap-2 rounded-lg bg-gray-600 py-2 px-3 font-semibold text-white transition-all hover:bg-gray-700 disabled:bg-gray-400">
//                   {isMessaging ? (<div className="flex items-center gap-2"><div className="h-5 w-5 animate-spin rounded-full border-2 border-white border-t-transparent"></div><span>Sending...</span></div>) : (<><MessageSquare size={16} /><span>Send & Proceed to Payment</span></>)}
//                 </button>
//               </div>
//             )}
//             {showPaymentOptions && cart.length > 0 && (
...
//       switch (messageType) {
//         case 'finalizeBill': templateName = "invoice_with_payment"; bodyParameters = [merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
//         case 'cashPayment': templateName = "payment_receipt_cashhhh"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
//         case 'qrPayment': templateName = "payment_receipt_upiii"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
//         case 'cardPayment': templateName = "payment_receipt_card"; bodyParameters = [orderId, merchantName, `₹${totalAmount.toFixed(2)}`, itemsList]; break;
//         default: throw new Error(`Invalid message type: ${messageType}`);
//       }
...
//     <>
//       {/* --- MODIFICATION START --- */}
//       {/* This is the main container. `h-screen` makes it full height, `overflow-hidden` prevents the body from scrolling. */}
//       <div className="flex flex-col h-screen bg-[#f9f9fb] font-sans overflow-hidden">
//         {/* Header: `flex-shrink-0` prevents it from shrinking. It is a static part of the flex column. */}
...
//         {/* Main Content: `flex-1` makes it take all available space. `overflow-y-auto` makes ONLY this section scrollable. */}
//         <main className="flex-1 overflow-y-auto p-4 space-y-3">
//           {/* --- MODIFICATION END --- */}
...
//         {/* --- MODIFICATION START --- */}
//         {/* Footer: `flex-shrink-0` prevents it from shrinking. It sits statically at the bottom of the flex column. */}
