...
// // Cart item type (keeps local id)
// type CartItem = {
//   id: number; // local cart id (Date.now())
//   productId?: number; // inventory product id if from inventory
//   name: string;
//   quantity: number;
//   price: number; // selling price per unit
// };

// // Inventory product shape (matches your Inventory page)
...
//   // Autocomplete: update suggestions when productName changes
//   useEffect(() => {
//     if (!productName) {
//       setSuggestions([]);
...
//   // add to cart (generic) - if productId provided, will attach it
...
//     // directly add to cart (or you could let user hit Add)
//     addToCart(p.name, p.sellingPrice, p.id);
//   };

//   // manual Add button handler (for entries not in inventory)
//   const handleManualAdd = () => {
//     if (!productName || productPrice <= 0) {
//       alert('Enter name and valid price');
//       return;
//     }
//     // try to find exact inventory match to attach productId
//     const match = inventory.find((inv) => inv.name.toLowerCase() === productName.trim().toLowerCase());
//     if (match) {
//       addToCart(match.name, match.sellingPrice, match.id);
//     } else {
//       addToCart(productName.trim(), productPrice, undefined);
//     }
//   };
...
//   // Barcode scanner update: try to match to inventory and add
...
//     if (!isNaN(asNum)) {
//       found = inventory.find((p) => p.id === asNum);
//     }
//     if (!found) {
//       // try by name match (exact or includes)
//       const lower = scannedValue.toLowerCase();
//       found = inventory.find((p) => p.name.toLowerCase() === lower) ?? inventory.find((p) => p.name.toLowerCase().includes(lower));
//     }
//     if (found) {
//       addToCart(found.name, found.sellingPrice, found.id);
//     } else {
//       // fallback: add scanned string as name with placeholder price (0) — you may change behavior
//       addToCart(scannedValue, 0, undefined);
//     }
//     setScanning(false);
//   };

//   // Example UPI URL (replace with real payee details)
//   const upiQR = `upi://pay?pa=example@upi&pn=YourCompany&am=${totalAmount.toFixed(2)}&cu=INR`;
...
//         {/* Manual Add Product with Autocomplete */}
//         <div className="mb-6 p-4 rounded-lg bg-white shadow-sm relative">
//           <h2 className="text-lg font-semibold mb-2">Add Item Manually</h2>
...
//                 type="text"
//                 placeholder="Product Name"
//                 className="border p-2 rounded w-full focus:ring-[#5a4fcf] focus:border-[#5a4fcf]"
//                 value={productName}
//                 onChange={(e) => {
...
//                 <div className="absolute z-50 bg-white border mt-1 w-full rounded shadow max-h-56 overflow-auto">
//                   {suggestions.map((s) => (
//                     <button
//                       key={s.id}
...
//                         <div className="text-xs text-gray-500">₹{s.sellingPrice}</div>
//                       </div>
//                       <div className="text-xs text-gray-400">{s.quantity} in stock</div>
//                     </button>
//                   ))}
//                 </div>
//               )}
...
//               type="number"
//               placeholder="Price"
//               className="border p-2 rounded w-32 focus:ring-[#5a4fcf] focus:border-[#5a4fcf]"
//               value={productPrice || ''}
//               onChange={(e) => setProductPrice(parseFloat(e.target.value) || 0)}
//             />
//             <button
...
//             Tip: start typing product name to see suggestions from Inventory.
//           </div>
//         </div>
...
//         <div className="space-y-3">
//           <h2 className="text-lg font-semibold mb-2">Cart Items</h2>
//           {cart.length === 0 && <p className="text-gray-500">No items in the cart.</p>}
//           {cart.map((product) => (
//             <div
//               key={product.id}
//               className="flex flex-wrap sm:flex-row sm:items-center justify-between border p-3 rounded-lg bg-white shadow-sm"
...
//                       }}
//                       className={`block w-full text-center px-4 py-2 rounded mb-2 ${
//                         selectedPayment === method ? 'bg-green-500 text-white' : 'bg-gray-200'
//                       } hover:bg-green-400 transition-colors`}
//                     >
//                       {method}
//                     </button>
