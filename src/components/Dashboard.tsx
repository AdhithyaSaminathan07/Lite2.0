...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchSales = async () => {
      ...
      try {
        // =================================================================
        // THE FIX: Add the `credentials: 'include'` option to your fetch call.
        // This sends the user's session cookie to the API.
        // =================================================================
        const res = await fetch(`/api/sales?period=${activePeriod.toLowerCase()}`, {
          credentials: 'include',
        });
        ...
        if (!res.ok) {
            // This now handles the error properly if authentication fails.
        ...
        console.error("Failed to load sales data:", err);
        setSalesError((err as Error).message || "Could not load data.");
      } finally {
        setIsSalesLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchInventorySummary = async () => {
        ...
        setSummaryError(null);
        try {
            // It's good practice to also add credentials here for consistency.
            const res = await fetch('/api/products', {
                credentials: 'include',
            });
            if (!res.ok) throw new Error('Failed to fetch product data');
